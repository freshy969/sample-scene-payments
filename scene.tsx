...
export default class GenericPayment extends ScriptableScene<{}, IPaymentState> {
  ...
  async handlePayment() {
    try {
      ...
      console.log(
        `error trying to pay to ${receivingAddress} ${amount}${currency}`,
        e
      );
  ...
  async render() {
    ...
    const doorRotation = { x: 0, y: isDoorClosed ? 0 : -90, z: 0 };

    const statusVisible = userPaid || tx !== null;
    const statusFontSize = userPaid ? 50 : 40;
    const statusColor = userPaid ? "green" : "red";
    const statusMessage = userPaid
      ? "Enter!"
      : error
        ? "Failed to sign MetaMask transaction"
